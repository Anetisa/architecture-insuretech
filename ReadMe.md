...
7. В зависимости от выбранных средств интеграции подумайте, требуется ли где-то применение паттернов отказоустойчивости:
	...
	- Retry,
	- Timeout.   
Отобразите применение паттернов на схеме.
8. Примите во внимание, что сервисы задеплоены в нескольких экземплярах. Подумайте, зависит ли ваше решение от этого.

**Решение:**
- `Task4/InsureTech_C4_сontainer-diagram-osago.drawio.xml` - новый вариант схемы
...
**Решение:**
- `Task5/client-inf.graphql` - схема GraphQL
- `Task5/Task_5.md` - анализ Swagger контракта
...
**Задание:**
1. Доработайте конфигурационный файл Nginx таким образом, чтобы обеспечить ограничение количества отправляемых запросов. Должно быть не более 10 запросов в минуту.
2. Сделайте так, чтобы при превышении лимита запросов клиент получал HTTP-ошибку с кодом 429.


**Решение:**
- `Task6/nginx.conf` — конфигурационный файл Nginx.